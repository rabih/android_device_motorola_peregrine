...
if [ $COMMON_DEVICE != "" ]; then
    ...
    if [ -f ../../$VENDOR/$DEVICE/proprietary-files.txt ]; then
        extract ../../$VENDOR/$DEVICE/proprietary-files.txt $DEVICE_BASE
    fi
    # Extract the files common to all devices using this common device tree
    extract ../../$VENDOR/$DEVICE/common-proprietary-files.txt $COMMON_BASE
fi
